import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@novahub/sdk/v2"
import { SessionTurn } from "@novahub/ui/session-turn"
import { SessionReview } from "@novahub/ui/session-review"
import { DataProvider } from "@novahub/ui/context"
import { DiffComponentProvider } from "@novahub/ui/context/diff"
import { CodeComponentProvider } from "@novahub/ui/context/code"
import { WorkerPoolProvider } from "@novahub/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@novahub/ui/logo"
import { IconButton } from "@novahub/ui/icon-button"
import { ProviderIcon } from "@novahub/ui/provider-icon"
import { createDefaultOptions } from "@novahub/ui/pierre"
import { iife } from "@novahub/util/iife"
...
import { Tabs } from "@novahub/ui/tabs"
import { MessageNav } from "@novahub/ui/message-nav"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@novahub/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@novahub/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@novahub/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@novahub/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@novahub/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/novahub-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | NovaHub</Title>
              </Show>
              <Meta name="description" content="novahub - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/novahub"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
