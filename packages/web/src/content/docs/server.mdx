---
title: Server
description: Interact with novahub server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `novahub serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an novahub client can use.
...
```bash
novahub serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
novahub serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `NOVAHUB_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `novahub`, or set `NOVAHUB_SERVER_USERNAME` to override it. This applies to both `novahub serve` and `novahub web`.

```bash
NOVAHUB_SERVER_PASSWORD=your-password novahub serve
```
...
When you run `novahub` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the novahub server to interact with novahub programmatically.
:::

This architecture lets novahub support multiple clients and allows you to interact with novahub programmatically.

You can run `novahub serve` to start a standalone server. If you have the
novahub TUI running, `novahub serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NovaHub [IDE](/docs/ide) plugins.
...
The novahub server exposes the following APIs.
