---
title: Providers
description: Using any LLM provider in NovaHub.
---

import config from "../../../config.mjs"
export const console = config.console

NovaHub uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your NovaHub config.
...
in `~/.local/share/novahub/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your NovaHub
config.
...
NovaHub Zen is a list of models provided by the NovaHub team that have been
tested and verified to work well with NovaHub. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with NovaHub Zen.
:::

1. Run the `/connect` command in the TUI, select novahub, and head to [novahub.ai/auth](https://novahub.ai/auth).
...
It works like any other provider in NovaHub and is completely optional to use.
...
To use Amazon Bedrock with NovaHub:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running novahub:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY novahub

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile novahub

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX novahub
   ```
   ...
   For project-specific or persistent configuration, use `novahub.json`:

   ```json title="novahub.json"
   {
     "$schema": "https://novahub.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in NovaHub is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for novahub to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX novahub
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX novahub
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your novahub config.
...
to store token in novahub auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
NovaHub uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock NovaHub
to only use your own GitLab-hosted instance, add the following to your
`novahub.json` file. It is also recommended to disable session sharing.
...
More documentation on [novahub-gitlab-auth](https://www.npmjs.com/package/@gitlab/novahub-gitlab-auth) homepage.

##### Configuration

Customize through `novahub.json`:
...
{
  "$schema": "https://novahub.ai/config.json",
  "plugin": ["@gitlab/novahub-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with novahub:
...
To use Google Vertex AI with NovaHub:
...
2. Set the required environment variables:
   ...
   Set them while running novahub.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id novahub
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through novahub, you can always configure it yourself.
...
```jsonc title="~/.config/novahub/novahub.jsonc"
{
  "$schema": "https://novahub.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [novahub-helicone-session](https://github.com/H2Shami/novahub-helicone-session) plugin to automatically log each NovaHub conversation as a session in Helicone.

```bash
npm install -g novahub-helicone-session
```
...
```json title="novahub.json"
{
  "plugin": ["novahub-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each NovaHub conversation listed as a separate session.
...
You can configure novahub to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="novahub.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://novahub.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure novahub to use local models through LM Studio.

```json title="novahub.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://novahub.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure novahub to use local models through Ollama.

:::tip
Ollama can automatically configure itself for NovaHub. See the [Ollama integration docs](https://docs.ollama.com/integrations/novahub) for details.
:::

```json title="novahub.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://novahub.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with NovaHub:
...
3. Copy the API key for use in NovaHub.
...
6. **Important**: Before using cloud models in NovaHub, you must pull the model information locally:
...
NovaHub Zen is a list of tested and verified models provided by the NovaHub team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>NovaHub Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **NovaHub Zen**.

   ```txt
   /connect
   ```

3. Enter your NovaHub API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your novahub config.
...
5. You can also customize them through your novahub config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' novahub
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group novahub
   ```
...
You can also customize models through your novahub config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with novahub. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in novahub.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `novahub.json` file in your project directory:

   ```json title="novahub.json" ""myprovider"" {5-15}
   {
     "$schema": "https://novahub.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="novahub.json" {9,11,17-20}
{
  "$schema": "https://novahub.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow NovaHub to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `novahub auth list` to see if the credentials
...
2. For custom providers, check the novahub config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your novahub config.
