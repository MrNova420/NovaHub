...
:::caution
Modes are now configured through the `agent` option in the novahub config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in novahub allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the novahub config.
...
novahub comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.novahub/plans/*.md` to detail the plan itself
...
Configure modes in your `novahub.json` config file:
...
- Global: `~/.config/novahub/modes/`
- Project: `.novahub/modes/`

```markdown title="~/.config/novahub/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global novahub config and the project specific config.
...
Create mode files in `.novahub/modes/` for project-specific modes or `~/.config/novahub/modes/` for global modes:

```markdown title=".novahub/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/novahub/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
