...
You can add external tools to NovaHub using the _Model Context Protocol_, or MCP. NovaHub supports both local and remote servers.
...
You can define MCP servers in your [NovaHub Config](https://novahub.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="novahub.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/novahub` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NovaHub automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NovaHub will:
...
If the server requires authentication, NovaHub will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `novahub mcp auth <server-name>`.
...
```bash
novahub mcp auth my-oauth-server
```
...
```bash
novahub mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, NovaHub will store the tokens securely in `~/.local/share/novahub/mcp-auth.json`.
...
novahub mcp debug my-oauth-server
```
...
Your MCPs are available as tools in NovaHub, alongside built-in tools. So you can manage them through the NovaHub config like any other tool.
...
```json title="novahub.json" {11, 14-18}
{
  "$schema": "https://novahub.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
novahub mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect NovaHub to your Sentry account.
