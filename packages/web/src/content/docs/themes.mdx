...
With NovaHub you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NovaHub uses our own `novahub` theme.
...
NovaHub comes with several built-in themes.
...
- Want NovaHub to match their terminal's appearance
...
NovaHub supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/novahub/themes/*.json` or `$XDG_CONFIG_HOME/novahub/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.novahub/themes/*.json`
4. **Current working directory** - Defined in `./.novahub/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/novahub/themes
vim ~/.config/novahub/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .novahub/themes
vim .novahub/themes/my-theme.json
```
...
{
  "$schema": "https://novahub.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
