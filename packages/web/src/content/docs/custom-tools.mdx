---
title: Custom Tools
description: Create tools the LLM can call in novahub.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside novahub's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.novahub/tools/` directory of your project.
- Or globally, by placing them in `~/.config/novahub/tools/`.
...
```ts title=".novahub/tools/database.ts" {1}
...
```ts title=".novahub/tools/math.ts"
import { tool } from "@novahub/plugin"
...
```ts title=".novahub/tools/project.ts" {8}
import { tool } from "@novahub/plugin"
...
```python title=".novahub/tools/add.py"
import sys
...
```ts title=".novahub/tools/python-add.ts" {10}
import { tool } from "@novahub/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".novahub/tools/add.py")
