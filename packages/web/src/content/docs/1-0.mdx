...
description: What's new in NovaHub 1.0.
---

NovaHub 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same novahub server.
...
version. However some older versions of NovaHub always grab latest.

To upgrade manually, run

```bash
$ novahub upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ novahub upgrade 0.15.31
```
