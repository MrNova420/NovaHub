...
NovaHub has a list of keybinds that you can customize through the NovaHub config.
...
NovaHub uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The NovaHub desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `novahub.json`.
