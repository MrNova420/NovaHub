...
There are two types of agents in NovaHub; primary agents and subagents.
...
NovaHub comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
NovaHub comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

NovaHub comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `novahub.json` config file:
...
- Global: `~/.config/novahub/agents/`
- Per-project: `.novahub/agents/`

```markdown title="~/.config/novahub/agents/review.md"
---
...
If no temperature is specified, NovaHub uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global NovaHub config and the project specific config.
...
The model ID in your NovaHub config uses the format `provider/model-id`. For example, if you're using [NovaHub Zen](/docs/zen), you would use `novahub/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="novahub.json" {3-6,9-12}
{
  "$schema": "https://novahub.ai/config.json",
  "tools": {
    "write": true,
...
```json title="novahub.json" {3-5,8-10}
{
  "$schema": "https://novahub.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `novahub models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/novahub).
:::
...
```markdown title="~/.config/novahub/agents/docs-writer.md"
---
...
```markdown title="~/.config/novahub/agents/security-auditor.md"
---
