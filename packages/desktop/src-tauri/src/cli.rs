...
const CLI_INSTALL_DIR: &str = ".novahub/bin";
const CLI_BINARY_NAME: &str = "novahub";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("novahub-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        ...
        .env("NOVAHUB_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("NOVAHUB_EXPERIMENTAL_FILEWATCHER", "true")
        .env("NOVAHUB_CLIENT", "desktop")
