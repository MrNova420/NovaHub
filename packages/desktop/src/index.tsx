...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@novahub/app"
...
import { Splash } from "@novahub/ui/logo"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NOVAHUB__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://novahub.ai/favicon-96x96-v3.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`novahub:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__NOVAHUB__ ??= {}
            window.__NOVAHUB__.serverPassword = data().password ?? undefined
