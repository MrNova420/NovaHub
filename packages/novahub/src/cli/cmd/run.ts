...
import { createOpencodeClient, type OpencodeClient } from "@novahub/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run novahub with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running novahub server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.NOVAHUB_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://novahub.internal", fetch: fetchFn })
