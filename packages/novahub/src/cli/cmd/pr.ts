...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run novahub",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for novahub session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found novahub session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`novahub import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting novahub...")
        UI.println()

        // Launch novahub TUI with session ID if available
        const { spawn } = await import("child_process")
        const novahubArgs = sessionId ? ["-s", sessionId] : []
        const novahubProcess = spawn("novahub", novahubArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          novahubProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`novahub exited with code ${code}`))
          })
          novahubProcess.on("error", reject)
        })
      },
    })
  },
