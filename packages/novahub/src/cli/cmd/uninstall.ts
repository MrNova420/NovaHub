...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall novahub and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall NovaHub")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g novahub",
      pnpm: "pnpm uninstall -g novahub",
      bun: "bun remove -g novahub",
      yarn: "yarn global remove novahub",
      brew: "brew uninstall novahub",
      choco: "choco uninstall novahub",
      scoop: "scoop uninstall novahub",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall novahub -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using NovaHub!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# novahub") || content.includes(".novahub/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".novahub/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".novahub/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".novahub"))
    ) {
      continue
    }
