...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for novahub.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal novahub sessions
...
novahub acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via novahub's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | novahub acp
```
...
ACP sessions map cleanly to novahub's internal session model:
