...
import type { Event, OpencodeClient, SessionMessageResponse } from "@novahub/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `novahub auth login` in the terminal",
        name: "Login with novahub",
        id: "novahub-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // NovaHub stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const novahubProvider = providers.find((p) => p.id === "novahub")
    if (novahubProvider) {
      if (novahubProvider.models["big-pickle"]) {
        return { providerID: "novahub", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(novahubProvider.models))
      if (best) {
        return {
