...
const managedConfigDir = process.env.NOVAHUB_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "novahub.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "novahub.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "novahub.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "novahub.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .novahub directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const novahubDir = path.join(dir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })
      const agentDir = path.join(novahubDir, "agent")
...
test("loads agents from .novahub/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const novahubDir = path.join(dir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })

      const agentsDir = path.join(novahubDir, "agents")
...
test("loads commands from .novahub/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const novahubDir = path.join(dir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })

      const commandDir = path.join(novahubDir, "command")
...
test("loads commands from .novahub/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const novahubDir = path.join(dir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })

      const commandsDir = path.join(novahubDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "novahub.json"),
        JSON.stringify({ $schema: "https://novahub.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "novahub.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .novahub config
      const projectDir = path.join(dir, "project")
      const novahubDir = path.join(projectDir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })
      ...
      // Local .novahub config with different plugins
      await Bun.write(
        path.join(novahubDir, "novahub.json"),
        JSON.stringify({
          $schema: "https://novahub.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .novahub config with some overlapping plugins
      await Bun.write(
        path.join(novahubDir, "novahub.json"),
        JSON.stringify({
          $schema: "https://novahub.ai/config.json",
...
// Note: preload.ts sets NOVAHUB_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .novahub config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .novahub directory config enables it
      const novahubDir = path.join(dir, ".novahub")
      await fs.mkdir(novahubDir, { recursive: true })
      await Bun.write(
        path.join(novahubDir, "novahub.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/novahub")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/novahub")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-novahub@2.4.3")).toBe("oh-my-novahub")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@novahub/plugin@2.0.0")).toBe("@novahub/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-novahub@2.4.3", "file:///project/.novahub/plugin/oh-my-novahub.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.novahub/plugin/oh-my-novahub.js")
  })
  ...
  test("local plugin directory overrides global novahub.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const novahubDir = path.join(projectDir, ".novahub")
        const pluginDir = path.join(novahubDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("NOVAHUB_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"]
    process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .novahub/ directories when flag is set", async () => {
    const originalEnv = process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"]
    process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .novahub directory with a command
          const novahubDir = path.join(dir, ".novahub", "command")
          await fs.mkdir(novahubDir, { recursive: true })
          await Bun.write(path.join(novahubDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .novahub should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NOVAHUB_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NOVAHUB_CONFIG_DIR"]
      process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NOVAHUB_CONFIG_DIR"]
      } else {
        process.env["NOVAHUB_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NOVAHUB_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NOVAHUB_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NOVAHUB_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NOVAHUB_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from NOVAHUB_CONFIG_DIR, not project
