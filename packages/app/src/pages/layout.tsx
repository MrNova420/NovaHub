...
import { base64Encode } from "@novahub/util/encode"
import { decode64 } from "@/utils/base64"
import { Avatar } from "@novahub/ui/avatar"
import { ResizeHandle } from "@novahub/ui/resize-handle"
import { Button } from "@novahub/ui/button"
import { Icon } from "@novahub/ui/icon"
import { IconButton } from "@novahub/ui/icon-button"
import { InlineInput } from "@novahub/ui/inline-input"
import { Tooltip, TooltipKeybind } from "@novahub/ui/tooltip"
import { HoverCard } from "@novahub/ui/hover-card"
import { MessageNav } from "@novahub/ui/message-nav"
import { DropdownMenu } from "@novahub/ui/dropdown-menu"
import { Collapsible } from "@novahub/ui/collapsible"
import { DiffChanges } from "@novahub/ui/diff-changes"
import { Spinner } from "@novahub/ui/spinner"
import { Dialog } from "@novahub/ui/dialog"
import { getFilename } from "@novahub/util/path"
import { Session, type Message, type TextPart } from "@novahub/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@novahub/ui/toast"
...
import { useDialog } from "@novahub/ui/context/dialog"
import { useTheme, type ColorScheme } from "@novahub/ui/theme"
...
export default function Layout(props: ParentProps) {
  ...
  const deepLinkEvent = "novahub:deep-link"

  const parseDeepLink = (input: string) => {
    if (!input.startsWith("novahub://")) return
  ...
  const drainDeepLinks = () => {
    const pending = window.__NOVAHUB__?.deepLinks ?? []
    if (pending.length === 0) return
    if (window.__NOVAHUB__) window.__NOVAHUB__.deepLinks = []
  ...
  const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
    ...
    const novahub = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === novahub ? "https://novahub.ai/favicon.svg" : props.project.icon?.override}
              ...
              <div class="overflow-y-auto max-h-72 h-full">
                <MessageNav
                  ...
                  onMessageSelect={(message) => {
                    if (!isActive()) {
                      sessionStorage.setItem("novahub.pendingMessage", `${props.session.id}|${message.id}`)
          ...
          <div class="shrink-0 w-full pt-3 pb-3 flex flex-col items-center gap-2">
            ...
            <Tooltip placement={sidebarProps.mobile ? "bottom" : "right"} value={language.t("sidebar.help")}>
              <IconButton
                ...
                variant="ghost"
                size="large"
                onClick={() => platform.openLink("https://novahub.ai/desktop-feedback")}
