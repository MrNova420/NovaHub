...
import { createOpencode, type ToolPart } from "@novahub/sdk"
...
console.log("🚀 Starting novahub server...")
const novahub = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await novahub.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new novahub session...")
    const { client, server } = novahub
    ...
    console.log("✅ Created novahub session:", createResult.data.id)
  ...
  console.log("📝 Sending to novahub:", message.text)
